        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">
                <section class="content-header">
                    <h1>
                        DHL
                        <small>Manage all pending DHL Requests</small>
                    </h1>
                    <ol class="breadcrumb">
                            ...
                            <div class="box">
                                <div class="box-header">
                                   ...
                                   <div class="box-tools">
                                        <div class="input-group" style="text-align: right;">
                                            {!! Form::open(['method' => 'POST' , 'action' => 'DHLSearchController@store']) !!}
                                ...
                                <div class="box-body table-responsive no-padding">
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            @if($DHLTransactions)
                                                @foreach($DHLTransactions as $DHLTransaction)
                                                    <tr>
                                                        <td>{{  $DHLTransaction->TransactionCode }}</td>
                                                        <td>{{  $DHLTransaction->searchInput }}</td>
                                                        <td>{{  $DHLTransaction->recieverFullName }}</td>
                                                        <td>{{  $DHLTransaction->Branch }}</td>
                                                        <td>{{  $DHLTransaction->recieverDepartment }}</td>
                                                        <td>
                                                            <form action="">
                                                                <input type="checkbox" name="acknowledge" id="acknowledge">           <input type="submit" name="submit" value="Ack" class="btn btn-info">
                                                            </form>
                                                        </td>
                                                        <td><a href="{{ route('dhl.edit',  $DHLTransaction->id) }}" class="btn btn-warning">Edit</a>
                                                            {!! Form::open(['method' => 'delete' , 'route' => ['dhl.destroy' , $DHLTransaction->id]]) !!}
